...
export class psdStyleClass implements AfterViewInit, OnDestroy {
	...
	resolveTarget() {
		...
		switch (this.selector) {
			...
			default: {
				return this.selector.startsWith('@shadow')
					? this.el.nativeElement.getRootNode().querySelector(this.selector.substr(7))
	...
	bindDocumentListener() {
		if (!this.documentListener) {
			const rootNode = this.selector.startsWith('@shadow')
				? this.el.nativeElement.getRootNode() : this.el.nativeElement.ownerDocument;
			this.documentListener = this.renderer.listen(rootNode, 'click', event => {
				// TODO: const target = event.target.shadowRoot ? event.path[0] : event.target;
				if (getComputedStyle(this.target).getPropertyValue('position') === 'static') {
					this.unbindDocumentListener();
				} else if (!this.el.nativeElement.isSameNode(event.target)
					&& !this.el.nativeElement.contains(event.target)
					&& !this.target.contains(event.target)
					&& !event.target.isSameNode(event.target.shadowRoot)) {
					this.leave();
				}
			});
		}
